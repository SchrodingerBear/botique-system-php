					...
					<div class="menu-desktop">
						<ul class="main-menu">
							...
							<li>
								...
								<?php else: ?>
									<a href="profile.php" style="font-weight: bolder; color:#fa4251;">Hello,
							...
							<div class="p-r-50 p-t-5 p-lr-0-lg">
								...
								<div class="p-t-33">
									<div class="flex-w flex-r-m p-b-10">
										...
										<div class="size-204 respon6-next">
											<div class="rs1-select2 bor8 bg0">
												<select required class="js-select2 size_text<?php echo $items['id'] ?>"
													name="time">
									...
									<div class="flex-w flex-r-m p-b-10">
										...
										<div class="size-204 respon6-next">
											<div class="rs1-select2 bor8 bg0">
												<select required class="js-select2 color_text<?php echo $items['id'] ?>"
													name="time">
													<option>Choose an option</option>
									...
									<div class="flex-w flex-r-m p-b-10">
										...
										<div class="size-204 respon6-next">
											<div class="rs1-select2 bor8 bg0">
												<select disabled class="js-select2 color_text<?php echo $items['id'] ?>"
													name="time">
													<option><?php echo $items['quantity']; ?></option>
									...
									<div class="flex-w flex-r-m p-b-10">
										<div class="size-204 flex-w flex-m respon6-next">
											...
											<!-- Add to Cart Button -->
											<button
												class="flex-c-m stext-101 cl0 size-101 bg1 bor1 hov-btn1 p-lr-15 trans-04 js-addcart-detail<?php echo $items['id'] ?>">
												Add to cart
											</button>
										</div>
									</div>
	...
	<?php foreach ($getallProduct as $item): ?>
		<script>
			$('.js-addcart-detail<?php echo $item['id'] ?>').each(function () {
				var nameProduct = $(this).parent().parent().parent().parent().find('.js-name-detail<?php echo $item['id'] ?>').html();
				$(this).on('click', function () {
					// Get the selected values
					const size = $(`.size_text<?php echo $item['id']; ?>`).val();
					const color = $(`.color_text<?php echo $item['id']; ?>`).val();
					const stockQuantity = <?php echo $item['quantity']; ?>; // Get stock quantity

					// Validate required fields
					if (size === "Choose an option" || !size) {
						swal("Size not selected", "Please choose a size before adding to the cart.", "warning");
						return; // Stop execution if size is not selected
					}

					if (color === "Choose an option" || !color) {
						swal("Color not selected", "Please choose a color before adding to the cart.", "warning");
						return; // Stop execution if color is not selected
					}

					// If stock is 0, prevent adding to cart
					if (stockQuantity == 0) {
						swal("No Available Stocks", "This product is out of stock.", "warning");
